...
**Config linking (`config/`):** app configs (kitty, wezterm, ghostty, alacritty, mako, fastfetch, mise, starship.toml) are symlinked into `~/.config/` via `link_repo_config_path`, and only if the corresponding directory/file exists in the repo — new configs should follow this same guarded pattern rather than being unconditionally linked. KDE Plasma dotfiles (`dolphinrc`, `kcminputrc`, `kdeglobals`, `plasmarc`, etc.) are **not** managed by this repo — they churn too often from live desktop state to be worth tracking in git, so `setup.sh`/`uninstall.sh` don't touch them.
